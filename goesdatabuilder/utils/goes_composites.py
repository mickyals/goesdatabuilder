...
BAND_INFO = {
    ...
    3: {
        ...
        "limitations": [
            ...
            "Clouds over land are less distinct than over water because both land and clouds are reflective at 0.86 um",
        ],
    },
    4: {
        "name": "Cirrus",
...
def print_recipe(name: str) -> None:
    ...
    if name in COMPOSITES_RGB:
        ...
        all_keys = list(COMPOSITES_RGB.keys()) + list(COMPOSITES_DIFFERENCE.keys()) + [str(b) for b in BAND_INFO.keys()]
        raise KeyError(f"Unknown product '{name}'. Available: {', '.join(all_keys)}")
